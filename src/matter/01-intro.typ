...
The *Ehrenpreis--Palamodov (EP)* principle of #cite(<ehrenpreis>, form: "prose") provides a constructive route
to such priors for *linear PDEs with constant coefficients*,
inspired by the *inverse Fourier transform*.
...
Its GP realization, the *Ehrenpreis--Palamodov Gaussian Process (EPGP)* of #cite(<harkonen>, form: "prose"),
...
Its plane-wave prior spans exactly the source-free solutions of the interior Maxwell problem,
matching the scattered field it must represent.
The surrogate is built with the Maxwell EPGP library *`maxwellgp`* from the ongoing work of #cite(<felix>, form: "prose").
...
- Cavity EPGP solver:
  Built a probabilistic EPGP solver `cavity-epgp` for the cavity reaction operator in Python using `maxwellgp`, relying on the analytic dipole Green's function.
- Cavity BEM solver:
...
- Analytic spherical cavity solution:
  Assembled the closed-form reaction operator for the spherical cavity from the standard vector spherical harmonic expansion, serving as exact ground truth.
- Benchmarking:
