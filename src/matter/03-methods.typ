...
Both represent the scattered field as a superposition of exact solutions of the interior curl--curl equation,
so Maxwell's equations hold by construction and only the PEC boundary condition remains to be enforced.
Beyond this shared structure, the two solvers are numerically independent:
they share only the problem setup (cavity geometry, wavenumber, dipole configurations) and none of their internal discretizations.

EPGP is probabilistic, returning a posterior distribution over the solution space.
It uses a volume plane-wave superposition ansatz and enforces the boundary condition by conditioning on boundary data at scattered collocation points,
with no mesh required.
BEM is deterministic, returning a single solution.
It discretizes the cavity wall into a surface mesh, represents the field via a boundary single-layer potential,
and enforces the boundary condition by solving a boundary integral equation.
...
It places a Gaussian prior on the plane-wave amplitude density and enforces the boundary condition by conditioning on the boundary data,
yielding a posterior distribution over the solution space.

=== Fundamental Principle

A linear PDE with constant coefficients takes the form $L uv = 0$,
where $L = L[partial_1, dots, partial_n]$ is a matrix-valued polynomial in the partial derivatives.
The Fourier transform turns differentiation into multiplication, $partial_j |-> i k_j$,
so $L$ acts on plane waves by its symbol,
$
...
It states that _all_ solutions $uv$ of linear PDEs with constant coefficients
...
Rather than carry the kernel constraint, we remove it by projection.
Writing $amat(Pi)_kv$ for the orthogonal projector onto $ker P(kv)$,
any unconstrained weight $avec(w)(kv)$ yields an admissible amplitude $av(kv) = amat(Pi)_kv avec(w)(kv)$.
...
$
Because the projector sits inside every feature, the prior is supported entirely on the solution space:
every sample satisfies Maxwell exactly.
...
It trades off fitting the data against trusting the prior:
as $sigma_n^2 -> 0$ the posterior mean interpolates the observations exactly,
while $sigma_n^2 > 0$ relaxes this to a regression that smooths the fit.
...
The posterior of the previous section is exact but not yet computable:
the kernel entries are integrals over $V_k$ with no closed form.
We discretize the EP integral by quadrature,
which both makes the kernel computable and expresses the field through explicit finite features.

==== Finite Spectral Features

We approximate the integral over $V_k$ by a finite sum over $N_s$ spectral directions $kv_j in V_k$,
drawn from a Fibonacci sphere, a quasi-uniform low-discrepancy point set.
For each direction we pick an orthonormal basis $av_(j 1), av_(j 2)$ of the transverse plane,
recovering the projector as an outer-product sum,
$
...
These coefficients carry the prior.
Gathering them in $avec(w) in CC^F$, the Gaussian measure becomes a finite complex Gaussian,
$
...
The function-space view of the previous section conditions through the kernel:
it forms the $N_b times N_b$ Gram block $amat(K)_(b b)$ at the observation points and solves against it.
The explicit features open a second, weight-space view that solves for the coefficient vector $avec(w)$ directly.
The two are the same posterior, and the cheaper one depends on which dimension is smaller.

Evaluating the features at the observation points through $cal(R)$ gives the design matrix $amat(Phi) = amat(Phi)(X_b) in CC^(F times N_b)$,
through which the data act on the coefficients.
The posterior over the coefficients is again Gaussian, with mean $avec(w)_star$ and precision $amat(A)$,
$
  avec(w)_star = amat(A)^(-1) amat(Phi) hv \/ sigma_n^2
  quad
  amat(A) = amat(W)^(-1) + amat(Phi) amat(Phi)^herm \/ sigma_n^2 in CC^(F times F).
$
The matrix $amat(A)$ is the posterior precision in weight space:
the prior precision $amat(W)^(-1)$ regularized by the data contribution $amat(Phi) amat(Phi)^herm \/ sigma_n^2$.
The posterior mean field follows by mapping the coefficients back through the features, $Ev_star (xv) = amat(Phi)(xv)^herm avec(w)_star$.
...
$
Substituting this discretized kernel into the function-space mean and applying the Woodbury matrix-inversion lemma reproduces the weight-space mean exactly.
The lemma trades the $N_b times N_b$ inverse of the function-space solve for the $F times F$ inverse $amat(A)^(-1)$,
so we may solve whichever is smaller.
The EP construction makes the feature count $F = 2 N_s$ explicit and often modest,
while the boundary value problem conditions on many points.
Whenever $F < N_b$ the weight-space solve is the cheaper one,
with a cost set by the number of features rather than the number of observations.

==== Hyperparameters

The model has three hyperparameters:
the spectral directions $kv_j$, the prior weights $amat(W)$, and the regularization parameter $sigma_n$.
...
since its marginal-likelihood optimum is governed by the condition number of the system rather than by the data.
...
while the plane-wave features, the prior weights, the directions, and the posterior solve are identical.
...
evaluated at $N_b$ points on the boundary $partial D$, each carrying its outward unit normal $nn$.

Although the prescribed boundary data is exact, we keep $sigma_n^2 > 0$.
The finite feature space cannot represent the trace exactly, and the plane-wave Gram matrix is ill-conditioned,
...
The `cavity-epgp` layer specializes the BVP solver to the cavity scattering problem.
...
The posterior covariance depends only on the conditioning points, not on the boundary values,
so it is shared by all transmitters and computed once.
...
The boundary element method (BEM) is a deterministic, boundary-integral method @colton @buffa.
Its ansatz is a single-layer potential, which satisfies Maxwell's equations exactly.
It leaves the surface density unknown and enforces the boundary condition by solving the resulting electric field integral equation,
yielding a single solution.
...
$
where $amat(G)$ is the free-space electric dyadic Green's function of the Helmholtz curl--curl equation
and $avec(j): partial D -> T (partial D)$ is a tangential boundary density.
...
In the indirect formulation,
the electric field is represented by a Maxwell single-layer potential $Psi_"SL"$ with an unknown density $avec(j)$:
$
  Ev = Psi_"SL" avec(j).
$
...
and the BVP solution operator is then
$
...
Notice the trace asymmetry:
the density is solved against the rotated trace $gamma_times$ on the cavity boundary $partial D$,
but the measurement reads the tangential projection trace $pi_t^Lambda$ on the dipole surface $Lambda$.

==== Reaction Operator

Each of the $M = 2 N_Lambda$ transmitter dipoles, two polarizations per point on $Lambda$,
...
$
and the EFIE is tested against the same basis.
The continuous equation $cal(V) avec(j) = avec(h)_times$ becomes a linear system for the coefficients,
$
  amat(V) jv = bv
  quad
  bv_i = integral_(partial D) avec(phi)_i dot avec(h)_times dif s,
$
in which the single-layer operator $cal(V)$ is replaced by its Galerkin matrix $amat(V)$.
Its entries are the single-layer bilinear form on pairs of basis functions.
Integrating the gradients of the dyadic Green's function by parts onto the basis functions
reduces it to a double surface integral over the scalar fundamental solution $Phi$,
$
...
Bembel uses div-conforming isogeometric B-splines, the higher-order spline analogue of Raviart--Thomas elements,
matching the $H^(-1/2)(div_Gamma, partial D)$ trace space of $cal(V)$.
