...
the governing field equations, the cavity geometry, and the reaction operator that both solvers compute.
...
It radiates an analytically known incident field $Ev^i$.
The incident field hits the cavity boundary $partial D$ and is reflected by it,
creating an unknown scattered field $Ev^s$.
The scattered field is read back by another dipole $delta_r$ acting as the receiver.

The total field is the superposition of the incident and scattered part,
$
  Ev = Ev^i + Ev^s,
$
where the scattered field satisfies the source-free curl--curl equation in the interior,
while the incident field and the total field only satisfy it away from the source.
...
An oscillating monopole $delta^1 = (zv, q)$ is a charge source $rho(xv, t) = q delta_zv exp(-i omega t)$ at a point $zv in D$,
with a charge $q in RR$ that gives its strength.
...
An oscillating Hertzian dipole $delta^2 = (zv, pv)$ is a current source $Jv(xv, t) = pv delta_zv exp(-i omega t)$ at a point $zv in D$,
together with a polarization $pv in RR^3$ that gives the dipole's orientation and strength.
...
$
where $rn rn^transp$ is the outer product of the unit separation vector $rn := rv \/ r$
and $amat(I)$ is the $3 times 3$ identity matrix.
...
by projecting it onto its polarization $pv_r$, yielding a single complex scalar.
...
the transmitter generates the incident field, and the receiver measures the scattered field.
...
This is the Lorentz (Rayleigh--Carson) reciprocity property @colton,
a consequence of the symmetry of the curl--curl Green's operator,
$
...
with locations $zv in Lambda$ and polarizations $pv in T_zv Lambda$ tangent to it.
...
By linearity of the problem,
the density excites a single incident field that superposes the fields of its constituent dipoles,
$
...
We sample $N_Lambda$ points on $Lambda$ and assign each an orthonormal tangent frame ${en_1(zv), en_2(zv)}$,
computed from the outward normal via a reference-vector construction.
...
writing $delta_i = (zv_i, pn_i)$ for the dipole with location $zv_i$ and unit polarization $pn_i$.
Consecutive pairs share a location, the two polarizations at one point.
...
$
This matrix is the object of interest for the benchmark.
Both solvers compute it, and the resulting operators are compared.

== Geometry

We now fix the concrete geometry of our benchmark problem.
The geometry is mostly taken from @cavity.
...
The ellipsoidal cavity is the original geometry from @cavity,
...
The ellipsoidal cavity is the original geometry from @cavity.
It is a smooth, convex, and simply connected domain.
...
The spherical cavity is a new addition that allows for an analytic solution.
It is a smooth, convex, and simply connected domain.
...
We refer to @tai for the full derivation and record the resulting formulas below.

The interior fields expand in the regular Hansen multipoles,
the TM-type $avec(N)_(l m)$ and the TE-type $avec(M)_(l m)$.
