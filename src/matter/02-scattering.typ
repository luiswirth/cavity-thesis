...
Its explicit form #cite(<tai>, supplement: [Ch. 4]) is given by
$
...
They enforce that the scattered field reflects the incident field such that their tangential components cancel on the cavity wall.
...
$
assigning a polarization vector $avec(g)(zv)$ to each point of $Lambda$.
...
The scalar reaction is this kernel as a bilinear form in the two polarization vectors, one slot per dipole,
$
...
$
with the spherical Bessel function $j_l$ and the Riccati--Bessel function
$psi_l (x) = x j_l (x)$ and its derivative $psi'_l$.
...
The PEC condition $pi_t Ev^s = -pi_t Ev^i$ on the cavity wall
$partial D$ at $r = R$ fixes the coefficients,
$
...
Projecting onto the receiver polarization vector gives the reference reaction operator,
$
