...
On the spherical cavity, where the analytic operator is available, BEM matches the ground truth to $approx 3 times 10^(-12)$ and EPGP to $approx 1.3 times 10^(-10)$ in relative Frobenius-norm error $epsilon$, certifying each independently. On the ellipsoidal cavity, where no analytic solution exists, the two solvers agree to about $10^(-8)$ in the same relative Frobenius norm. Since both were already certified on the sphere, this agreement carries over to a geometry with no ground truth. The agreement of a boundary-integral and a spectral plane-wave method, which share no numerical machinery, validates the EPGP on this geometry and certifies the reaction operator as a robust reference benchmark.

Beyond matching the reference, the EPGP satisfies the time-harmonic Maxwell equations exactly by construction, needs no surface mesh, and provides a posterior uncertainty estimate as a byproduct. It is also the cheaper solver at any demanding tolerance, reaching a given reciprocity error two to three orders of magnitude faster than the BEM in our experiments. This speed comparison should be read with the caveats in the Limitations: the BEM was run unoptimized, without fast-multipole acceleration or matrix compression, so the absolute factor mixes implementation quality with algorithmic cost. The benchmark thus establishes the EPGP as a Maxwell-consistent, uncertainty-aware surrogate for the interior cavity reaction operator.

== Limitations

==== Validation Scope

The validation rests most firmly on the spherical cavity, where each solver is checked directly against the analytic operator. On the ellipsoidal cavity there is no ground truth, so the evidence is the agreement between the two unrelated solvers, which is mutual consistency rather than a direct measure of accuracy. The reciprocity error cannot close this gap, since it constrains only the antisymmetric part of the error. Splitting the error operator $amat(T) - amat(T)_"ref"$ into a symmetric and an antisymmetric part, the reciprocity error $rho = norm(amat(T) - amat(T)^transp) \/ norm(amat(T))$ depends only on the antisymmetric part: the true operator is symmetric, so a symmetric component of the error leaves the operator symmetric and the reciprocity error does not see it. A solver could therefore carry a large symmetric error while its reciprocity error stays small, so a small reciprocity error is necessary but not sufficient for accuracy.
...
The scale of the uncertainty is set by the assumed noise $sigma_n$, so one might try to calibrate it by tuning $sigma_n$ to the data. This does not help. Optimizing $sigma_n$ by maximizing the marginal likelihood drives it toward a floor rather than toward the true error level. The plane-wave features are overcomplete and strongly correlated, so as $sigma_n$ decreases the precision matrix $amat(A) = amat(W)^(-1) + amat(Phi) amat(Phi)^herm \/ sigma_n^2$ becomes increasingly dominated by the ill-conditioned feature term $amat(Phi) amat(Phi)^herm \/ sigma_n^2$. Its conditioning then approaches that of the feature Gram matrix $amat(Phi) amat(Phi)^herm$, whose spectrum sets the limiting condition number, rather than following a clean $1 \/ sigma_n^2$ law. Below a certain noise level the system becomes too ill-conditioned to solve stably, and the floating-point floor, rather than the data, determines the smallest usable $sigma_n$.
A more principled quadrature might relieve this, though, as noted below, better integration accuracy does not by itself guarantee a better-conditioned feature matrix.
...
The EPGP draws its wavevector directions from a Fibonacci sphere, a quasi-Monte Carlo rule. A more principled choice is the Lebedev quadrature #cite(<lebedev>), which integrates spherical harmonics exactly up to a chosen degree. We expect it to reach the same kernel accuracy with fewer directions, giving a smaller system. Whether it also yields a better-conditioned system is an open question: good integration accuracy does not by itself imply a well-conditioned feature matrix, since the conditioning is governed by the correlations among the plane-wave features rather than by the accuracy of the quadrature. Should the conditioning improve, the lower condition number would in turn let the assumed noise drop further, which is the main obstacle to calibrated uncertainty.
