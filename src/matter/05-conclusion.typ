...
The validation rests most firmly on the spherical cavity, where each solver is checked directly against the analytic operator. On the ellipsoidal cavity there is no ground truth, so the evidence is the agreement between the two unrelated solvers, which is mutual consistency rather than a direct measure of accuracy. The reciprocity error cannot close this gap, since it constrains only the antisymmetric part of the error. #hl[Writing the error operator $amat(T) - amat(T)_"ref"$ as the sum of a symmetric and an antisymmetric part, the reciprocity error $rho = norm(amat(T) - amat(T)^transp) \/ norm(amat(T))$ depends only on the antisymmetric part: the true operator is symmetric, so any symmetric component of the error leaves $amat(T)$ symmetric and is invisible to $rho$. A solver could therefore have a large symmetric error while reporting a small $rho$, which is why a small reciprocity error is necessary but not sufficient for accuracy.]
