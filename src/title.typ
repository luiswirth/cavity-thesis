...
#block(
    ...
    *Abstract*\
    ...
    We reconstruct this operator with the EPGP
    and compare it against an independent boundary element method (BEM) reference using Bembel.
    We investigate two cavity geometries.
    On the spherical cavity, where the reaction operator is available in closed form,
    both solvers match it to between $10^(-10)$ and $10^(-12)$.
    On the ellipsoidal cavity, which admits no analytic solution,
    the EPGP agrees with the BEM reference to about $10^(-8)$.
