...
| C12 | §4.2.1 Uncertainty | "The lobes sit where the field hits the wall most steeply, so its normal component is large …" | **REMOVED** (KURZ 39). Investigated numerically: the posterior variance depends only on the conditioning geometry, so the uncertainty is identical for every transmitter (rel. diff = 0) and the normal-component explanation is refuted (alignment corr 0.77 for one transmitter, 0.39 for another). No *proven* mechanistic interpretation of the lobe pattern exists (a single-mode "near-resonance" picture also fails, corr 0.30). Per decision, the thesis states only the facts — pattern, scale, geometry-determined, determinability-not-error — and drops the normal-component thread entirely (on the sphere too). The full investigation and figure are recorded here for the paper, not in the thesis. |
